...
;(function ($) {
    ...
    Iconpicker.prototype.bindEvents = function () {
        ...
        op.table.find('.btn-previous, .btn-next').off('click').on('click', function (e) {
            ...
            var per_page = 0;
            if (el.options.rows === 0) {
                per_page = el.options.icons.length;
            }
            else {
                per_page = el.options.cols * el.options.rows;
            }
            var total = Math.ceil(el.options.icons.length / per_page);
            if (op.page + inc < 1 || op.page + inc > total) {
                return;
            }
    ...
    Iconpicker.prototype.updateSearch = function () {
        ...
        var search = [
            '<tr>',
            '   <td colspan="' + op.cols + '">',
            '       <input type="text" name="search" class="form-control search-control" style="width: 100%;" placeholder="' + op.searchText + '">',
            '   </td>',
            '</tr>'
        ];
        search = $(search.join(''));
