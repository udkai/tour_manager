...
public class WebTag {
     ...
     *//*
         * public static List<SysModule> getCurrentModule(HttpServletRequest
		 * request) { UserSession userSession =
		 * SessionUtil.getUserSession(request); SysRoleService sysRoleService =
     ...
     * @return
     */
    public static String getUserType(Integer type) {
        if (type == 1)
            return "管理员";
        else if (type == 2)
            return "管理教员";
     ...
     * @return
     */
    public static String createWarn(HttpServletRequest request) {
        SysRoleService sysRoleService = SpringContextHolder.getBean("sysRoleService");
        return sysRoleService.createWarnStr();
    }
}
