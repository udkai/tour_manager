...
public class SysModuleDao {
	...
	public int add(SysModule sysModule) {
		...
		String sql = "insert into t_sys_module(name,code,parent_id,url,target,iconImg,icon_color,display_order) " +
				"values(:name,:code,:parent_id,:url,:target,:iconImg,:icon_color,:display_order)";
	...
	public int update(SysModule sysModule) {
		String sql = "update t_sys_module set name=:name,code=:code,url=:url,parent_id=:parent_id,target=:target,iconImg=:iconImg," +
				"icon_color=:icon_color,display_order=:display_order where id=:id";
		NamedParameterJdbcTemplate namedParameterJdbcTemplate = new NamedParameterJdbcTemplate(jdbcTemplate);
		SqlParameterSource paramSource = new BeanPropertySqlParameterSource(sysModule);
		return namedParameterJdbcTemplate.update(sql, paramSource);
	}
